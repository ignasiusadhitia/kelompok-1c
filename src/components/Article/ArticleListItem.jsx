...
const ArticleListItem = ({ data, fetchArticles }) => {
  ...
  const handleNavigationToEdit = () => {
    navigate(`/dashboard/articles/edit/${data.id}`, {
      state: data,
    });
  };
  ...
  const handleDelete = (id) => {
    deleteData(`/api/article/${id}`, fetchArticles)
  };

  return (
    <tr className="border-b dark:border-gray-700">
      <td className="px-4 py-3">{data.date}</td>
      <td className="px-4 py-3"><span className="line-clamp-3">{data.title}</span></td>
      <td className="px-4 py-3"><span className="line-clamp-3 text-justify"><div dangerouslySetInnerHTML={{ __html: data.content }} /></span></td>
          ...
          >
            <li>
              <button
                ...
                onClick={() => handleNavigationToDetail(data.id)}
              >
                Detail
              </button>
            </li>
            <li>
              <button
                className="w-full text-start py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                onClick={() => handleNavigationToEdit(data.id)}
              >
                Edit
              </button>
            </li>
